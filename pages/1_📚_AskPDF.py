...
def main():
    ...
    st.set_page_config(page_title="📚 AskPDF", page_icon="📚")
    ...
    if "text_detector" not in st.session_state:
        st.session_state.text_detector = False
    ...
    st.markdown('''<p style="color:red;">PDF-less? We've got you covered. Snag an awesome sample with one click above!</p>''', unsafe_allow_html=True)
    st.subheader('Your PDFs')

    pdf_docs = st.file_uploader(
        "Drop files, click Process, watch the AI work its magic!", accept_multiple_files=True)
    ...
    if pdf_docs is not None and 1 <= len(pdf_docs) <= 3:
        ...
        if st.session_state.conversation:
            user_question = st.text_input(
                "Delve into the insights within your PDFs:", placeholder="Ask me anything...", key="user_input_before")
            if user_question:
        ...
        st.warning(
            "*** For PDF reprocessing or adding additional files, simply hit that refresh button! ***")
